...
SEED_BASEURL = _seed_setting if _seed_setting else ''
...
IPTV_ORG_INDEX_CACHE = os.path.join(_KODI_TEMP, 'dltv_iptv_org_idx2.json')
IPTV_ORG_TVG_CACHE = os.path.join(_KODI_TEMP, 'dltv_iptv_org_tvg.json')
IPTV_ORG_TTL = 7 * 24 * 3600  # refresh index every 7 days
_XMLTV_TTL = 4 * 3600      # re-download every 4 hours
_XMLTV_PROG_CACHE = os.path.join(_KODI_TEMP, 'dltv_xmltv_programs.json')
_XMLTV_PROG_TTL = 1800     # re-parse every 30 minutes
_XMLTV_SOURCES = {
    'epg_fr': ('French',      'https://epgshare01.online/epgshare01/epg_ripper_FR1.xml.gz', 'dltv_xmltv_FR.xml.gz'),
    'epg_uk': ('English',     'https://epgshare01.online/epgshare01/epg_ripper_UK1.xml.gz', 'dltv_xmltv_UK.xml.gz'),
    'epg_ar': ('Arabic',      'https://epgshare01.online/epgshare01/epg_ripper_AR1.xml.gz', 'dltv_xmltv_AR.xml.gz'),
    'epg_es': ('Spanish',     'https://epgshare01.online/epgshare01/epg_ripper_ES1.xml.gz', 'dltv_xmltv_ES.xml.gz'),
    'epg_it': ('Italian',     'https://epgshare01.online/epgshare01/epg_ripper_IT1.xml.gz', 'dltv_xmltv_IT.xml.gz'),
    'epg_de': ('German',      'https://epgshare01.online/epgshare01/epg_ripper_DE1.xml.gz', 'dltv_xmltv_DE.xml.gz'),
    'epg_pt': ('Portuguese',  'https://epgshare01.online/epgshare01/epg_ripper_PT1.xml.gz', 'dltv_xmltv_PT.xml.gz'),
    'epg_nl': ('Dutch',       'https://epgshare01.online/epgshare01/epg_ripper_NL1.xml.gz', 'dltv_xmltv_NL.xml.gz'),
    'epg_tr': ('Turkish',     'https://epgshare01.online/epgshare01/epg_ripper_TR1.xml.gz', 'dltv_xmltv_TR.xml.gz'),
    'epg_pl': ('Polish',      'https://epgshare01.online/epgshare01/epg_ripper_PL1.xml.gz', 'dltv_xmltv_PL.xml.gz'),
    'epg_ro': ('Romanian',    'https://epgshare01.online/epgshare01/epg_ripper_RO1.xml.gz', 'dltv_xmltv_RO.xml.gz'),
    'epg_ru': ('Russian',     'https://epgshare01.online/epgshare01/epg_ripper_RU1.xml.gz', 'dltv_xmltv_RU.xml.gz'),
}
...
# extensions — all confirmed as real video by AES decryption. URL-based detection is not reliable.
...
def _fetch_stream_key(key_id, channel_key, state, key_cache, force_refresh=False, key_ns=None):
    ...
    server is always used — avoiding AES key mismatch.
    """
    ...
    try:
        ...
        if channel_salt and channel_salt != 'noauth':
            ...
            hdrs['Authorization'] = f'Bearer {state["auth_token"]}'
            hdrs['X-Key-Timestamp'] = str(ts)
            hdrs['X-Key-Nonce'] = str(nonce)
            hdrs['X-Key-Path'] = auth_sig
...
_FAV_REFRESH_TS_FILE = os.path.join(_KODI_TEMP, 'dltv_fav_refresh_ts')
...
LANG_NAMES = {
    ...
    'ita': 'Italian', 'por': 'Portuguese', 'ara': 'Arabic', 'nld': 'Dutch',
    'tur': 'Turkish', 'rus': 'Russian', 'pol': 'Polish', 'ron': 'Romanian',
    'swe': 'Swedish', 'nor': 'Norwegian', 'dan': 'Danish', 'fin': 'Finnish',
    'ces': 'Czech', 'hrv': 'Croatian', 'srp': 'Serbian', 'bul': 'Bulgarian',
    'hun': 'Hungarian', 'ell': 'Greek', 'heb': 'Hebrew', 'fas': 'Persian',
    'urd': 'Urdu', 'msa': 'Malay', 'hin': 'Hindi', 'ben': 'Bengali',
    'tam': 'Tamil', 'ind': 'Indonesian', 'zho': 'Chinese', 'tha': 'Thai',
    'vie': 'Vietnamese', 'jpn': 'Japanese', 'kor': 'Korean',
}
...
_lovecdn_url_cache = {}         # stream_name -> (lovecdn_url, timestamp)
_lovecdn_url_cache_lock = threading.Lock()
_LOVECDN_URL_TTL = 480          # 8 min (token valid ~15 min)
...
def _get_session():
    ...
    if not hasattr(_tls, 'session') or getattr(_tls.session, 'verify', True) is not False:
        s = requests.Session()
        s.verify = False  # CHEVY proxy uses self-signed/mismatched SSL certs
        _tls.session = s
    return _tls.session


def _fix_lovecdn_pcr(data, seg_dur):
    """Fix corrupt PCR values in a lovecdn TS segment.
    Some lovecdn segments have random/garbage PCR on the video PID while PTS/DTS
    remain correct. Kodi's native VideoPlayer uses PCR as the presentation clock,
    causing severe stuttering when PCR jumps wildly within a segment.
    Fix: anchor to the first valid PCR and linearly interpolate all subsequent
    PCR packets across the segment duration. The first PCR in lovecdn segments is
    always correct (matches PTS), so this preserves the PCR/PTS relationship."""
    n_pkts = len(data) // 188
    if n_pkts < 2:
        return data

    # Find the first PCR packet and its value
    first_pcr = None
    first_pcr_idx = 0
    for i in range(n_pkts):
        ...
        if data[off] != 0x47:
            continue
        if not ((data[off + 3] >> 4) & 1):  # no adaptation field
            continue
        if data[off + 4] < 7:  # adaptation too short
            continue
        if not (data[off + 5] & 0x10):  # no PCR_flag
            continue
        b = data[off + 6:off + 12]
        pcr_base = (b[0] << 25) | (b[1] << 17) | (b[2] << 9) | (b[3] << 1) | (b[4] >> 7)
        pcr_ext = ((b[4] & 1) << 8) | b[5]
        first_pcr = pcr_base * 300 + pcr_ext
        first_pcr_idx = i
        break

    if first_pcr is None:
        return data  # no PCR in segment, nothing to fix

    # Linearly interpolate PCR from first_pcr to first_pcr + seg_dur * 27MHz
    total_27mhz = seg_dur * 27000000
    data = bytearray(data)
    for i in range(first_pcr_idx, n_pkts):
        off = i * 188
        if data[off] != 0x47:
            continue
        if not ((data[off + 3] >> 4) & 1):
            continue
        if data[off + 4] < 7:
            continue
        if not (data[off + 5] & 0x10):
            continue
        frac = (i - first_pcr_idx) / max(n_pkts - first_pcr_idx, 1)
        pcr = first_pcr + int(frac * total_27mhz)
        pcr_base = (pcr // 300) & 0x1FFFFFFFF  # 33-bit
        pcr_ext = int(pcr % 300)               # 9-bit
        data[off + 6] = (pcr_base >> 25) & 0xFF
        data[off + 7] = (pcr_base >> 17) & 0xFF
        data[off + 8] = (pcr_base >> 9) & 0xFF
        data[off + 9] = (pcr_base >> 1) & 0xFF
        data[off + 10] = ((pcr_base & 1) << 7) | 0x7E | ((pcr_ext >> 8) & 1)
        data[off + 11] = pcr_ext & 0xFF
    return bytes(data)


def _lovecdn_get_media_url(stream_name, force_refresh=False):
    """Fetch and return the current lovecdn media playlist content.
    Returns (media_url, lt_url, media_text, media_base) or None on failure.
    Caches the token URL for _LOVECDN_URL_TTL seconds (8 min)."""
    now = time.time()
    lovecdn_url = lt_url = None

    if not force_refresh:
        with _lovecdn_url_cache_lock:
            cached = _lovecdn_url_cache.get(stream_name)
            if cached and now - cached[1] < _LOVECDN_URL_TTL:
                lovecdn_url, lt_url = cached[0], cached[2]

    if not lovecdn_url:
        with _lovecdn_url_cache_lock:
            expired = _lovecdn_url_cache.get(stream_name)
        lt_url = (expired[2] if expired and len(expired) > 2 else None) \
                 or f'https://lovetier.bz/player/{stream_name}'
        try:
            r = _lovecdn_session.get(lt_url, headers={'User-Agent': UA}, timeout=8)
        except Exception as e:
            log(f'[LovecdnProxy] lovetier fetch error for {stream_name}: {e}')
            return None
        if r.status_code != 200:
            log(f'[LovecdnProxy] lovetier HTTP {r.status_code} for {stream_name}')
            return None
        m3 = re.search(r'streamUrl\s*:\s*"(https?:[^"]+)"', r.text)
        if not m3:
            log(f'[LovecdnProxy] no streamUrl for {stream_name}')
            return None
        lovecdn_url = m3.group(1).replace('\\/', '/')
        with _lovecdn_url_cache_lock:
            _lovecdn_url_cache[stream_name] = (lovecdn_url, now, lt_url)
        log(f'[LovecdnProxy] refreshed token for {stream_name}')

    try:
        r2 = _lovecdn_session.get(lovecdn_url, headers={'User-Agent': UA, 'Referer': lt_url}, timeout=8)
    except Exception as e:
        log(f'[LovecdnProxy] media fetch error for {stream_name}: {e}')
        return None
    if r2.status_code != 200:
        log(f'[LovecdnProxy] lovecdn HTTP {r2.status_code} for {stream_name}')
        with _lovecdn_url_cache_lock:
            _lovecdn_url_cache.pop(stream_name, None)
        return None

    master_base = lovecdn_url.split('?')[0].rsplit('/', 1)[0] + '/'
    media_text = r2.text
    media_base = master_base
    current_url = lovecdn_url
    # Follow master→variant chain
    while '#EXT-X-STREAM-INF' in media_text:
        next_url = None
        preferred_url = None
        for line in media_text.splitlines():
            stripped = line.strip()
            if stripped and not stripped.startswith('#'):
                candidate = stripped if stripped.startswith('http') else urljoin(media_base, stripped)
                if not next_url:
                    next_url = candidate
                if not preferred_url and 'a1' in candidate:
                    preferred_url = candidate
        next_url = preferred_url or next_url
        if not next_url or next_url == current_url:
            break
        try:
            rv = _lovecdn_session.get(next_url, headers={'User-Agent': UA, 'Referer': lt_url}, timeout=8)
        except Exception as e:
            log(f'[LovecdnProxy] variant fetch error: {e}')
            return None
        if rv.status_code != 200:
            log(f'[LovecdnProxy] variant HTTP {rv.status_code}')
            return None
        current_url = next_url
        media_text = rv.text
        media_base = current_url.split('?')[0].rsplit('/', 1)[0] + '/'
    # Cache the resolved media playlist URL
    with _lovecdn_url_cache_lock:
        cached_entry = _lovecdn_url_cache.get(stream_name)
        if cached_entry:
            _lovecdn_url_cache[stream_name] = (current_url, now, cached_entry[2])

    return current_url, lt_url, media_text, media_base


def _lovecdn_prefetch_seg(seg_url):
    """Pre-download a lovecdn segment into memory cache (called in background thread)."""
    with _lovecdn_seg_cache_lock:
        if seg_url in _lovecdn_seg_cache:
            return  # already cached
    try:
        r = _lovecdn_session.get(seg_url, headers={'User-Agent': UA}, timeout=(3, 10))
        if r.status_code == 200:
            with _lovecdn_seg_cache_lock:
                _lovecdn_seg_cache[seg_url] = r.content
                # Keep cache bounded: drop oldest entries beyond 6
                while len(_lovecdn_seg_cache) > 6:
                    _lovecdn_seg_cache.pop(next(iter(_lovecdn_seg_cache)))
...
def _apply_custom_dns():
    ...
    if _test_ip:
        ...
        xbmc.log(f'[DLTV][CustomDNS] Active — using {_CUSTOM_DNS}', xbmc.LOGINFO)
    else:
        _active_dns = _DNS_FALLBACK
        xbmc.log(f'[DLTV][CustomDNS] {_CUSTOM_DNS} unreachable — using {_DNS_FALLBACK} as primary', xbmc.LOGWARNING)
...
_CREDS_JS_SKIP_HOSTS = (
    'code.jquery.com', 'cdn.jsdelivr.net', 'cdnjs.cloudflare.com',
    'ajax.googleapis.com', 'www.google.com', 'www.googletagmanager.com',
    'static.cloudflareinsights.com',
)

def _extract_creds_with_js(page_text, page_url, base_url, sess):
    """Extract authToken+channelSalt from page HTML and any referenced external JS files.
    Skips known CDN libraries (jQuery, Clappr, HLS.js, reCAPTCHA…) — they never contain credentials."""
    at = _extract_credential(page_text, 'authToken')
    cs = _extract_credential(page_text, 'channelSalt')
    if at and cs:
        return at, cs
    # Credentials may be in an external JS file loaded by the page
    for sm in re.finditer(r'<script[^>]+src=["\']([^"\']+)["\']', page_text):
        src = sm.group(1)
        if src.startswith('//'):
            src = 'https:' + src
        elif src.startswith('/'):
            ...
            continue
        if any(h in src for h in _CREDS_JS_SKIP_HOSTS):
            continue
        try:
            ...
            if at and cs:
                log(f'[EPlayerAuth] Credentials found in external JS: {src[:80]}')
                return at, cs
        except Exception as e:
            log(f'[EPlayerAuth] External JS fetch error ({src[:60]}): {e}')
    return None, None


def _is_js_challenge(text):
    """Return True if response looks like a Cloudflare/bot JS challenge page."""
    t = text[:800]
    return ("window['" in t and 'document.location' in t) or 'challenge-form' in t
...
def _fetch_auth_credentials(channel_id, max_attempts=3):
    ...
    Tries _KSOHLS_BASE first (with stream page referer); if it returns a JS challenge,
    falls back to _KSOHLS_FALLBACK_BASES."""
    # Use the stream page URL as referer — the auth server checks it.
    # Also visit it first: CF shares clearance state across zones on the same account,
    # so a prior visit to the configured base URL unblocks segments in the same session.
    stream_referer = abs_url(f'stream/stream-{channel_id}.php')
    try:
        _get_session().get(stream_referer, headers={'User-Agent': UA}, timeout=4)
    except Exception:
        pass
    bases = [_KSOHLS_BASE] + [b for b in _KSOHLS_FALLBACK_BASES if b != _KSOHLS_BASE]
    for base in bases:
        url = f'{base}/premiumtv/daddyhd.php?id={channel_id}'
        for attempt in range(max_attempts):
            try:
                ...
                if r.status_code in (502, 503, 504):
                    log(f'[EPlayerAuth] Server error {r.status_code} on {base} (attempt {attempt+1}/{max_attempts})')
                    if attempt < max_attempts - 1:
                        time.sleep(1.5)
                    continue
                if _is_js_challenge(r.text):
                    log(f'[EPlayerAuth] JS challenge on {base}, trying next')
                    break  # skip remaining retries for this base
                auth_token, channel_salt = _extract_creds_with_js(r.text, url, base, _get_session())
                if auth_token and channel_salt:
                    return auth_token, channel_salt
                log(f'[EPlayerAuth] Credentials not found on {base} (attempt {attempt+1}/{max_attempts})')
            except Exception as e:
...
def _discover_server_urls():
    ...
    Updates module-level globals and saves a cache file. Safe to call from a background thread."""
    global CHEVY_PROXY, CHEVY_LOOKUP, _KSOHLS_BASE, PLAYER_REFERER
    try:
        log('[Discovery] Starting server URL discovery...')
        watch_url = abs_url('watch.php?id=1')
        stream_url = abs_url('stream/stream-1.php')
        r = requests.get(stream_url, headers={'User-Agent': UA, 'Referer': watch_url}, timeout=8)
        if r.status_code != 200:
            log(f'[Discovery] stream page HTTP {r.status_code}')
            return

        # Find the auth player iframe (premiumtv URL)
        mp = re.search(r'<iframe[^>]+src="(https?://([^/"]+)/premiumtv/[^"]+)"', r.text)
        if not mp:
            log('[Discovery] no premiumtv iframe found')
            return
        player_url, ksohls_host = mp.group(1), mp.group(2)
        ksohls_base = f'https://{ksohls_host}'

        # Fetch the auth player page and scan for CHEVY URLs
        r2 = requests.get(player_url, headers={'User-Agent': UA, 'Referer': stream_url}, timeout=8)
        if r2.status_code != 200:
            log(f'[Discovery] auth player HTTP {r2.status_code}')
            return

        if _is_js_challenge(r2.text):
            log(f'[Discovery] auth player page behind JS challenge — skipping ksohls={ksohls_base}')
            return

        # Skip JS template literals like https://${M3U8_SERVER}/proxy/
        mp2 = re.search(r'(https?://(?!\$\{)[^\s"\'<>{}$]+)/proxy/', r2.text)
        chevy_proxy = mp2.group(1) if mp2 else None
        ml = re.search(r'(https?://(?!\$\{)[^\s"\'<>{}$]+)/server_lookup', r2.text)
        chevy_lookup = ml.group(1) if ml else None

        # If lookup not found as a literal, probe the proxy host's /health endpoint
        if chevy_proxy and not chevy_lookup:
            candidate = re.sub(r'^(https?://[^/]+).*', r'\1', chevy_proxy)
            try:
                rh = _get_session().get(f'{candidate}/health', headers={'User-Agent': UA}, timeout=4)
                if rh.status_code == 200:
                    chevy_lookup = candidate
                    log(f'[Discovery] lookup derived from proxy host: {chevy_lookup}')
            except Exception:
                pass

        # Fall back to current known lookup if still not resolved
    ...
    try:
        ...
        if os.path.exists(path):
            ...
            if file_mtime > in_memory_time and (time.time() - file_mtime) < 300:
                ...
                # New session from another process — reset dedup/stall so old
                # segment URLs don't cause immediate dedup skip.
                with _stall_lock:
                    _m3u8_stall.pop(channel_key, None)
                    _m3u8_timeouts.pop(channel_key, None)
                    _last_sent_seq.pop(channel_key, None)
...
def _refresh_channel_creds(cid, max_attempts=3):
    ...
    if direct_url:
        ...
        # Réinitialiser les compteurs de stall/timeout qui causent les saccades
        with _stall_lock:
            _m3u8_stall.pop(channel_key, None)
            _m3u8_timeouts.pop(channel_key, None)
            _last_sent_seq.pop(channel_key, None)
...
class _EPlayerProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if m:
            ...
            return
        m = re.match(r'^/ptv/([^/]+)/(.+)', self.path)
        if m:
            self._handle_premiumtv(m.group(1), m.group(2))
            return
        m = re.match(r'^/wide/([^/]+)/(.+)', self.path)
        if m:
            self._handle_wide(m.group(1), m.group(2))
            return
        m = re.match(r'^/lovecdn/([^/?]+)', self.path)
        if m:
            self._handle_lovecdn(unquote(m.group(1)))
            return
        m = re.match(r'^/lovecdn-ts/([^/?]+)', self.path)
        if m:
            self._handle_lovecdn_ts(unquote(m.group(1)))
            return
        m = re.match(r'^/lovecdn-seg/(.+)', self.path)
        if m:
            self._handle_lovecdn_seg(unquote(m.group(1)))
            return
    ...
    def _handle_m3u8(self, channel_key):
        ...
        if not state or not state.get('m3u8_url'):
            ...
            if m:
                ...
                if not state:
                    # Auth unavailable (NFS channel or ksohls page has no creds) — noauth fallback
                    log(f'[EPlayerProxy] Auth failed for {channel_key}, using noauth fallback')
                    noauth_url = resolve_stream_url(cid)
                    _set_channel_state(channel_key, 'noauth', 'noauth', noauth_url)
                    state = _get_channel_state(channel_key)
        if not state or not state.get('m3u8_url'):
        ...
        try:
            ...
            if r.status_code in (403, 404):
                ...
                if m:
                    ...
                    state = _refresh_channel_creds(cid)
                    if not state:
                        # Auth unavailable (NFS channel) — retry without auth header
                        log(f'[EPlayerProxy] re-auth failed for {channel_key}, retrying noauth')
                        noauth_url = resolve_stream_url(cid)
                        _set_channel_state(channel_key, 'noauth', 'noauth', noauth_url)
                        state = _get_channel_state(channel_key)
                    if state:
                        new_hdrs = dict(m3u8_hdrs)
                        at = state.get('auth_token', '')
                        if at and at != 'noauth':
                            new_hdrs['Authorization'] = f'Bearer {at}'
                        else:
                            new_hdrs.pop('Authorization', None)
                        try:
                            r = _get_session().get(state['m3u8_url'], headers=new_hdrs, timeout=3)
    ...
    def _handle_stream(self, channel_key):
        ...
        try:
            while not _proxy_abort.is_set() and not _client_gone.is_set():
                ...
                # Direct-HLS channels carry their own referer in state (premiumtv host).
                if state.get('player_referer') and state.get('player_referer') != _state_ref:
                    _state_ref = state['player_referer']
                    seg_hdrs = dict(_SEG_HEADERS)
                    seg_hdrs['Referer'] = _state_ref + '/'
                    seg_hdrs['Origin'] = _state_ref

                m3u8_hdrs = {
                    'User-Agent': _AUTH_UA,
                    'Referer': state.get('player_referer') or PLAYER_REFERER,
                    'Authorization': f'Bearer {state["auth_token"]}',
                    'X-Channel-Key': channel_key,
                    'X-User-Agent': _AUTH_UA,
                    'Cache-Control': 'no-cache',
    ...
    def _handle_key(self, channel_key, key_id, orig_srv_key=None):
        ...
        if not state or not state.get('m3u8_url'):
            ...
            if m:
                ...
                state = _refresh_channel_creds(cid)
                if not state:
                    log(f'[EPlayerProxy] Auth failed for key {channel_key}, using noauth fallback')
                    noauth_url = resolve_stream_url(cid)
                    _set_channel_state(channel_key, 'noauth', 'noauth', noauth_url)
                    state = _get_channel_state(channel_key)
        if not state or not state.get('m3u8_url'):
        ...
        try:
            ...
            if channel_salt and channel_salt != 'noauth':
                ...
                hdrs['X-Key-Path'] = auth_sig
                hdrs['X-Fingerprint'] = fp
            r = _get_session().get(key_url, headers=hdrs, timeout=3)
            body = r.content
            log(f'[EPlayerProxy] key {key_id}: {len(body)}B status={r.status_code} host={key_host}')
            self.send_response(200)
    ...
    def _handle_premiumtv(self, encoded_origin, encoded_url):
        """Serve the premiumtv media playlist for inputstream.adaptive.
        Fetches the live playlist with the premiumtv Referer, filters out the
        trailing relative-path segment (which the CDN returns 403 for, and which
        adaptive retries 10x then aborts on), and rewrites the absolute R2
        segment URLs to route through /raw/ so the Referer is applied."""
        try:
            origin = unquote(encoded_origin)
            raw_url = unquote(encoded_url)

            if not raw_url.startswith('http'):
                log(f'[PremiumTVProxy] relative URL rejected: {raw_url}')
                self.send_response(400)
                self.end_headers()
                return
            ...
            if r.content[:7] != b'#EXTM3U':
                # Not a manifest (segment or error body) — forward as-is
                body = r.content
                self.send_response(r.status_code)
            ...
            for line in r.text.splitlines():
                stripped = line.strip()
                if stripped and not stripped.startswith('#'):
                    # Keep only absolute http(s) segment URLs. Relative-path segments
                    # (e.g. 2026/08/19/...ts on the CDN host) 403 for everyone —
                    # filtering them lets adaptive start on a valid R2 segment.
    ...
    def _handle_wide(self, encoded_slug, encoded_url):
        """Serve a wideiptv (Player 6) HLS playlist/segment for inputstream.adaptive.
        Fetches with a fresh token (auto-refreshed via the wideiptv API), rewrites
        manifest segment/variant URLs through /wide/ so the token is re-applied."""
        try:
            slug = unquote(encoded_slug)
            raw_url = unquote(encoded_url)
            if not raw_url.startswith('http'):
                log(f'[WideIptvProxy] relative URL rejected: {raw_url}')
                self.send_response(400)
                self.end_headers()
                return
    ...
    def _handle_lovecdn(self, stream_name):
        """Proxy M3U8 for lovecdn streams with automatic token refresh.
        Segment URLs are rewritten to go through our local proxy (/lovecdn-seg/)
        which pre-fetches segments into memory — eliminates CDN latency and
        prevents HLS saccade at segment boundaries."""
        try:
            result = _lovecdn_get_media_url(stream_name)
            if not result:
                self.send_response(502)
                self.end_headers()
                return
            current_url, lt_url, media_text, media_base = result
            log(f'[LovecdnM3U8] {stream_name}: ' + ' | '.join(media_text.splitlines()[:20]))

            port = _actual_proxy_port or M3U8_PROXY_PORT
            lines = []
            for line in media_text.splitlines():
                stripped = line.strip()
                # Strip VOD/EVENT markers so libavformat treats stream as live (no loop)
                if stripped.startswith('#EXT-X-PLAYLIST-TYPE') or stripped == '#EXT-X-ENDLIST':
                    continue
                if stripped and not stripped.startswith('#'):
                    abs_seg = stripped if stripped.startswith('http') else urljoin(media_base, stripped)
                    # Route through local cache proxy
                    proxy_url = f'http://127.0.0.1:{port}/lovecdn-seg/{quote_plus(abs_seg)}'
                    lines.append(proxy_url)
                    # Pre-fetch in background so it's ready before Kodi asks for it
                    threading.Thread(
                        target=_lovecdn_prefetch_seg,
                        args=(abs_seg,),
                        daemon=True,
                    ).start()
                else:
            ...
            log(f'[LovecdnProxy] error for {stream_name}: {e}')
            try:
                self.send_response(502)
                self.end_headers()
            except Exception:
    ...
    def _handle_lovecdn_ts(self, stream_name):
        """Serve lovecdn stream as a continuous MPEG-TS byte stream (no HLS jitter).
        Acts as an internal HLS client: polls media playlist, downloads plain TS segments,
        and forwards them with null-packet pacing — same pattern as _handle_stream.

        Key design: HTTP response headers are NOT sent until 2 segments are buffered.
        This ensures ffmpegdirect/libavformat sees real H.264 SPS data immediately,
        avoiding DXVA2 initialization with incomplete stream info (which causes
        'status 2 with 255' errors when null packets are sent first)."""
        import queue as _q

        # Initial token fetch
        result = _lovecdn_get_media_url(stream_name, force_refresh=True)
        if not result:
            self.send_response(502)
            self.end_headers()
            return
        ...
        seen_urls = set()
        _seg_q = _q.Queue(maxsize=10)
        _client_gone = threading.Event()
        _NULL_PKT = bytes([0x47, 0x1F, 0xFF, 0x10]) + bytes(184)
        _PRE_BUFFER_MAX = 15.0
        _STALL_MAX = 30.0
        _last_content_time = [time.time()]

        def _download():
            """Download segments in background, filling _seg_q."""
            last_content = time.time()
            while not _proxy_abort.is_set() and not _client_gone.is_set():
                r = _lovecdn_get_media_url(stream_name)
                if not r:
                    _proxy_abort.wait(timeout=3)
                    r = _lovecdn_get_media_url(stream_name, force_refresh=True)
                    if not r:
                        log(f'[LovecdnTS] {stream_name}: token refresh failed, ending stream')
                        break

                _media_url, _lt_url, media_text, media_base = r
                tgt_m = re.search(r'#EXT-X-TARGETDURATION:(\d+)', media_text)
                tgt_dur = float(tgt_m.group(1)) if tgt_m else 6.0

                segs = []
                seg_dur_map = {}
                _last_inf = tgt_dur
                for _sl in media_text.splitlines():
                    _sl = _sl.strip()
                    if _sl.startswith('#EXTINF:'):
                        try:
                            _last_inf = float(_sl[8:].split(',')[0])
                        ...
                        _su = _sl if _sl.startswith('http') else urljoin(media_base, _sl)
                        segs.append(_su)
                        seg_dur_map[_su] = _last_inf
                        _last_inf = tgt_dur

                new_segs = [u for u in segs if u not in seen_urls]
                if not new_segs:
                    if time.time() - last_content >= _STALL_MAX:
                        log(f'[LovecdnTS] {stream_name}: stall >{int(_STALL_MAX)}s — ending stream')
                        break
                    _proxy_abort.wait(timeout=tgt_dur / 2)
                    continue

                for seg_url in new_segs:
                    if _proxy_abort.is_set() or _client_gone.is_set():
                        break
                    try:
                        sr = _get_session().get(seg_url, headers={'User-Agent': UA}, timeout=(3, 8))
                        if sr.status_code == 200:
                            ...
                            while not _client_gone.is_set() and not _proxy_abort.is_set():
                                try:
                                    ...
                                    pass
                            last_content = time.time()
                            _last_content_time[0] = last_content
                            log(f'[LovecdnTS] seg {len(body)}B dur={seg_dur:.1f}s {seg_url[-50:]}')
                        elif sr.status_code == 403:
                            log(f'[LovecdnTS] seg 403 — clearing token cache for {stream_name}')
                            with _lovecdn_url_cache_lock:
                                _lovecdn_url_cache.pop(stream_name, None)
                        else:
                            log(f'[LovecdnTS] seg {sr.status_code}: {seg_url[-50:]}')
                    except Exception as e:
                        log(f'[LovecdnTS] seg error: {e}')
            _seg_q.put(None)

        _dl_t = threading.Thread(target=_download, daemon=True)
        _dl_t.start()

        # Wait for 2 segments before sending HTTP response.
        # No null packets sent — ffmpegdirect must see real H.264 SPS immediately
        # so DXVA2 (or its fallback) initializes with correct stream parameters.
        _wait_start = time.time()
        while _seg_q.qsize() < 2 and not _proxy_abort.is_set():
            if time.time() - _wait_start > _PRE_BUFFER_MAX:
                log(f'[LovecdnTS] {stream_name}: pre-buffer timeout')
                _client_gone.set()
                _dl_t.join(timeout=5)
                self.send_response(504)
                self.end_headers()
                return
            time.sleep(0.05)

        log(f'[LovecdnTS] {stream_name}: {_seg_q.qsize()} segs ready, starting stream')
        try:
            self.send_response(200)
            self.send_header('Content-Type', 'video/MP2T')
            self.end_headers()
        except Exception as e:
            log(f'[LovecdnTS] {stream_name}: header error: {e}')
            _client_gone.set()
            _dl_t.join(timeout=5)
            return
        ...
        def _sender():
            ...
            while not _client_gone.is_set() and not _proxy_abort.is_set():
                ...
                try:
                    ...
                    log(f'[LovecdnTS] {stream_name}: client disconnected')
                    _client_gone.set()
                    _seg_q.task_done()
                    return
                now = time.time()
                _pace_clock = max(now, _pace_clock or now) + seg_dur
                _seg_q.task_done()
    ...
    def _handle_lovecdn_seg(self, seg_url):
        """Fetch a lovecdn segment or sub-playlist with proper headers.
        Serves from pre-fetch memory cache when available (instant, no CDN latency).
        If the response is an M3U8, segment URLs are rewritten through this proxy
        (handles variant/master playlists that contain relative segment paths)."""
        try:
            # Serve from memory cache if segment was pre-fetched
            with _lovecdn_seg_cache_lock:
                cached_body = _lovecdn_seg_cache.get(seg_url)
            if cached_body is not None:
                ...
                log(f'[LovecdnSeg] cache-hit {len(cached_body)}B {seg_url[-40:]}')
                return

            referer = 'https://lovetier.bz/'
            with _lovecdn_url_cache_lock:
                for sname, cached in list(_lovecdn_url_cache.items()):
                    if sname in seg_url and cached and len(cached) > 2 and cached[2]:
                        referer = cached[2]
                        break
            _p = urlparse(referer)
            origin = f'{_p.scheme}://{_p.netloc}'
            r = _lovecdn_session.get(seg_url, headers={
                'User-Agent': UA,
                'Referer': referer,
                'Origin': origin,
            }, timeout=10, stream=True)
            if r.status_code != 200:
                log(f'[LovecdnProxy] seg HTTP {r.status_code}: {seg_url[8:60]}')
                self.send_response(r.status_code)
                self.end_headers()
                return
            ct = r.headers.get('Content-Type', '')
            is_m3u8 = ('mpegurl' in ct or 'm3u8' in ct
                       or seg_url.split('?')[0].lower().endswith('.m3u8'))
            if is_m3u8:
                # Sub-playlist: buffer fully to detect HTML and rewrite segment URLs
                body_bytes = r.content
                body_text = body_bytes.decode('utf-8', errors='replace')
                if '<html' in body_text[:200].lower():
                    log(f'[LovecdnProxy] seg got HTML (Cloudflare?) for {seg_url[8:60]}')
                    with _lovecdn_url_cache_lock:
                        for sname in list(_lovecdn_url_cache.keys()):
                            if sname in seg_url:
                                _lovecdn_url_cache.pop(sname, None)
                                break
                    self.send_response(502)
                    self.end_headers()
                    return
                port = _actual_proxy_port or M3U8_PROXY_PORT
                base = seg_url.split('?')[0].rsplit('/', 1)[0] + '/'
                lines = []
                for line in body_text.splitlines():
                    stripped = line.strip()
                    if stripped and not stripped.startswith('#'):
                        abs_seg = stripped if stripped.startswith('http') else urljoin(base, stripped)
                        lines.append(f'http://127.0.0.1:{port}/lovecdn-seg/{quote_plus(abs_seg)}')
                    else:
                        lines.append(line)
                body = '\n'.join(lines).encode('utf-8')
                self.send_response(200)
                ...
                # TS segment: stream to Kodi in chunks — avoids buffering 2.5MB before first byte
                ...
                for chunk in r.iter_content(chunk_size=65536):
                    if chunk:
                        ...
                        total += len(chunk)
                log(f'[LovecdnProxy] seg ok {total}B {seg_url[8:60]}')
        except Exception as e:
            log(f'[LovecdnProxy] seg error: {e}')
            try:
                self.send_response(502)
                self.end_headers()
            except Exception:
            ...
            for attempt in range(2):
                ...
                try:
                    ...
                    if '407' in err_str or 'Proxy Authentication Required' in err_str:
                        log(f'[EPlayerProxy] seg proxy 407: incorrect proxy credentials — check the "Proxy segments" setting')
                        break
        ...
        try:
            ...
            if e.errno in (98, 10048):
                ...
                try:
                    ...
                    if rh.status_code == 200 and rh.text == 'dltv-proxy':
                        try:
...
def Main_Menu():
    menu = [
        ...
        ['[B][COLOR cyan]DLTV CHAT[/COLOR][/B]', 'chat', None],
    ]
...
def run_diagnostics():
    ...
    lines = [f'[B]DLTV — v{version} — Diagnostics[/B]', '']

    # 0. Configuration
    lines.append('[B]0. Configuration[/B]')
    lines.append(info(f'Base URL     : {get_active_base()}'))
    lines.append(info(f'CHEVY proxy  : {CHEVY_PROXY}'))
    lines.append(info(f'CHEVY lookup : {CHEVY_LOOKUP}'))
    dns = addon.getSetting('custom_dns').strip()
    lines.append(info(f'Custom DNS   : {dns or "(not set)"}'))
    seg_proxy = addon.getSetting('seg_proxy').strip()
    lines.append(info(f'Seg proxy    : {seg_proxy or "(not set)"}'))
    lines.append('')

    # 1. Channel list
    url = abs_url('24-7-channels.php')
    lines.append(f'[B]1. Channel list[/B]  ({url})')
    fetched = False
    for verify_ssl in [True, False]:
        try:
            ...
            if cards > 0:
                lines.append(ok(f'HTTP {r.status_code} — {cards} channels'))
                fetched = True
                break
            else:
                lines.append(ko(f'HTTP {r.status_code} — 0 channels (verify_ssl={verify_ssl})'))
        except Exception as e:
            lines.append(ko(f'verify_ssl={verify_ssl}: {type(e).__name__}: {str(e)[:80]}'))
    if not fetched:
        lines.append(ko('Failed to load channels'))
    lines.append('')

    # 2. CHEVY CDN — server lookup + m3u8 + AES key
    lines.append('[B]2. CHEVY CDN[/B]')
    # Use ch.116 (beIN SPORTS 1) as test channel — reliably on CHEVY
    _diag_cid = '116'
    _diag_key = f'premium{_diag_cid}'
    m3u8_content = ''
    try:
        _sl_r = requests.get(f'{CHEVY_LOOKUP}/server_lookup?channel_id={_diag_key}',
                             headers={'User-Agent': UA, 'Referer': PLAYER_REFERER}, timeout=6)
        _sk_d = _sl_r.json().get('server_key', '?') if _sl_r.status_code == 200 else '?'
        if _sk_d and _sk_d != 'nfs' and _sk_d != '?':
            lines.append(ok(f'server_lookup ch.{_diag_cid} → {_sk_d}'))
        elif _sk_d == 'nfs':
            lines.append(ko(f'server_lookup ch.{_diag_cid} → nfs (offline)'))
        else:
            lines.append(ko(f'server_lookup HTTP {_sl_r.status_code}'))
    except Exception as e:
        _sk_d = '?'
        lines.append(ko(f'server_lookup: {type(e).__name__}: {str(e)[:60]}'))

    # m3u8 fetch
    m3u8_url = resolve_stream_url(_diag_cid)
    try:
        _m3r = requests.get(m3u8_url, headers={'User-Agent': _AUTH_UA, 'Referer': PLAYER_REFERER},
                            timeout=8)
        m3u8_content = _m3r.text
        _segs_d = [l for l in m3u8_content.splitlines() if l and not l.startswith('#')]
        _ph_d, _real_d = 0, 0
        for _s in _segs_d:
            if _is_placeholder(_s):
                _ph_d += 1
            else:
                _real_d += 1
        if _m3r.status_code == 200 and _real_d > 0:
            lines.append(ok(f'm3u8 HTTP 200 — {_real_d} real segs ({_ph_d} filtered)'))
        elif _m3r.status_code == 200 and _ph_d > 0:
            lines.append(ko(f'm3u8 HTTP 200 — all {_ph_d} segs are placeholders'))
        else:
            lines.append(ko(f'm3u8 HTTP {_m3r.status_code} — {m3u8_content[:80]}'))
    except Exception as e:
        lines.append(ko(f'm3u8: {type(e).__name__}: {str(e)[:60]}'))

    # AES key fetch (no auth needed — CHEVY serves key directly)
    try:
        _km = re.search(r'/key/[^/]+/(\d+)', m3u8_content)
        if _km:
            _kid = _km.group(1)
            _kr = requests.get(f'{CHEVY_LOOKUP}/key/{_diag_key}/{_kid}',
                               headers={'User-Agent': _AUTH_UA, 'Referer': PLAYER_REFERER},
                               timeout=6)
            if _kr.status_code == 200 and len(_kr.content) == 16:
                lines.append(ok(f'AES key /key/{_diag_key}/{_kid} → 16B OK'))
            else:
                lines.append(ko(f'AES key HTTP {_kr.status_code} — {len(_kr.content)}B'))
        else:
            lines.append(info('AES key: no key URI in m3u8 (unencrypted or m3u8 failed)'))
    except Exception as e:
        lines.append(ko(f'AES key: {type(e).__name__}: {str(e)[:60]}'))
    lines.append('')

    # 3. Local proxy
    lines.append('[B]3. Local proxy[/B]')
    try:
        _ensure_m3u8_proxy()
        port = _actual_proxy_port or M3U8_PROXY_PORT
        rh = requests.get(f'http://127.0.0.1:{port}/health', timeout=2)
        if rh.status_code == 200 and rh.text == 'dltv-proxy':
            lines.append(ok(f'Proxy running — port {port}'))
            # Test m3u8 proxy path (noauth — works regardless of ksohls.ru)
            _set_channel_state(_diag_key, 'noauth', 'noauth', m3u8_url)
            rm = requests.get(f'http://127.0.0.1:{port}/m3u8/{_diag_key}', timeout=12)
            proxy_segs = [l for l in rm.text.splitlines() if l and not l.startswith('#')]
            if rm.status_code == 200 and proxy_segs:
                lines.append(ok(f'm3u8 proxy : HTTP 200 — {len(proxy_segs)} segments served'))
            else:
                lines.append(ko(f'm3u8 proxy : HTTP {rm.status_code} — {len(proxy_segs)} segments'))
        else:
            lines.append(ko(f'Proxy not responding (HTTP {rh.status_code})'))
    except Exception as e:
        lines.append(ko(f'Proxy: {type(e).__name__}: {str(e)[:80]}'))
    lines.append('')

    # 4. Stall state
    lines.append('[B]4. Stall state[/B]')
    ...
    if stall_keys:
        for k in stall_keys:
            ...
            lines.append(info(f'{k}: seq={st.get("seq")} frozen={_age}s retries={st.get("same_url_retries", 0)}'))
    else:
        lines.append(ok('No active stall'))
    lines.append(info(f'Cached channels: {", ".join(cred_keys) if cred_keys else "none"}'))
    lines.append('')

    # 5. CDN — CHEVY health
    lines.append('[B]5. CDN — CHEVY health[/B]')
    try:
        ...
        if _rh.status_code == 200:
            ...
            _daemon_str = 'connected' if _connected else 'DISCO'
            _health_line = f'Pool {_pool_str} — daemon {_daemon_str}{_fb_str} — sessions {_sess}'
            ...
            lines.append(info(f'Locator: {_sl.get("cache_size",0)} entries — hit {_sl.get("hit_rate_percent","?")}%{_bans_str}{_rl_str}'))
        else:
            lines.append(ko(f'Health HTTP {_rh.status_code}'))
    except Exception as _e5:
        lines.append(ko(f'Health: {type(_e5).__name__}: {str(_e5)[:60]}'))

    # 5b. Per-CDN freshness
    lines.append('')
    _CDN_DEFAULTS = {'ddy6': '773', 'wind': '464', 'zeko': '116'}
    ...
    for _sk3, _cid3 in sorted(_cdn_probe.items()):
        try:
            ...
            _parts3.append(f'age={_age_str3}')
            _det3 = '  '.join(_parts3)
            lines.append(ok(f'  {_sk3:<8} {_det3}  LIVE') if _vivant else
                         ko(f'  {_sk3:<8} {_det3}  FROZEN'))
        except Exception as _ex3:
            lines.append(ko(f'  {_sk3:<8} ERROR: {str(_ex3)[:60]}'))

    # 5c. Local CDN cache summary
    lines.append('')
    if os.path.exists(_CDN_CACHE_FILE):
        try:
            ...
            lines.append(info(f'Local cache: {len(_cdn2)} ch — age {_age_m2}min — {_sum2}'))
        except Exception:
            lines.append(info('Local CDN cache: corrupted'))
    else:
        lines.append(info('Local CDN cache: empty (auto-filled by ProbeBackground)'))

    msg = '\n'.join(lines)
    log(f'[Diagnostics]\n{msg}')
    xbmcgui.Dialog().textviewer(f'DLTV v{version} — Diagnostics', msg)
...
def TransList(categ, channels):
    ...
    for channel in channels:
        ...
        fav_label = '★ Remove from favorites' if channel_id in fav_ids else '☆ Add to favorites'
...
def toggle_favorite(cid, name):
    ...
    if cid in ids:
        ...
        save_favorites(favs)
        xbmcgui.Dialog().notification('DLTV', f'Removed from favorites: {name}', ICON, 2000)
    else:
        favs.append({'id': cid, 'name': name})
        save_favorites(favs)
        xbmcgui.Dialog().notification('DLTV', f'Added to favorites: {name}', ICON, 2000)
...
def list_favorites():
    favs = get_favorites()
    if not favs:
        xbmcgui.Dialog().notification('DLTV', 'No favorites. Long press a channel to add.', ICON, 3000)
        closeDir()
        return
    ...
    # EPG (epgshare01 XMLTV) has priority over the DLTV sports schedule
    ...
    addDir('[B][COLOR lime]+ Add channel by ID[/COLOR][/B]', build_url({'mode': 'add_by_id'}), False)
    for fav in sorted(favs, key=lambda x: x['name'].lower()):
        cid = fav['id']
        name = fav['name']
        cdn_name = _fav_cdn_map.get(cid) if cid in dead_cids else None
        ctx = [('Remove from favorites', 'RunPlugin(%s)' % build_url({'mode': 'toggle_fav', 'cid': cid, 'name': name}))]
        ev = ev_map.get(cid)
...
# Pass 1 — language/country words (≥3 chars) present anywhere in the channel name
_NAME_TO_LANG = {
    # Language adjectives
    'arabic': 'ara', 'arab': 'ara',
    ...
    'sudan': 'ara',
    'yemen': 'ara',
    # French country names (Canal+/beIN channels)
    'afrique': 'fra',   # Canal+ Sport Afrique → français
}

# Pass 1 — 2-letter ISO codes, checked ONLY as the last word of the name
_CODE_TO_LANG = {
...
def _lang_from_name(name):
    """Pass 1: detect language from explicit words in the channel name.
    - Last word: checked in _NAME_TO_LANG AND _CODE_TO_LANG.
    - Other words (right→left): _NAME_TO_LANG only (no 2-letter codes).
...
def _lookup_lang(name, index):
    """Return the language code for a channel.
    Pass 1: detection from name words (explicit country/language).
    Pass 2: lookup in iptv-org index with progressive normalizations."""
    ...
    # Normalize the name, then progressively strip country/language suffixes
    # to improve match chances in the index.
    ...
    # 2b. Strip trailing number ("Sky Sports 1" → "Sky Sports")
    no_num = re.sub(r'\s+\d+$', '', norm).strip()
    if no_num != norm and no_num in index:
        return index[no_num]

    # 2c. Strip the last word if it is a country/language indicator,
    #     then search again (with and without trailing number)
    parts = norm.split()
    if len(parts) > 1:
        last_word = parts[-1]
...
def _channel_name_to_tvg_id(name, tvg_index):
    """Map a DLTV channel name to an iptv-org tvg_id using progressive name stripping."""
    norm = _normalize_ch_name(name)
...
def _fetch_xmltv_fr_epg(cid_name_pairs):
    """Return {cid: current_program_title} from epgshare01 XMLTV sources selected in settings.

    Downloads each enabled language gz once every 4h, merges XML, caches results 30min.
    cid_name_pairs: list of (dltv_channel_id, channel_name)
    """
    ...
    # Determine enabled sources from settings
    enabled = [sid for sid in _XMLTV_SOURCES if addon.getSetting(sid).lower() == 'true']
    log(f'[xmltv] enabled sources: {enabled}')
    if not enabled:
        enabled = ['epg_fr']  # fallback to French if nothing selected

    # Download/cache each enabled source and merge XML
    xml_parts = []
    for sid in enabled:
        label, url, cache_fname = _XMLTV_SOURCES[sid]
        cache_file = os.path.join(_KODI_TEMP, cache_fname)
        xml_gz = None
        if os.path.exists(cache_file) and now_ts - os.path.getmtime(cache_file) < _XMLTV_TTL:
            try:
        ...
        if xml_gz is None:
            try:
                log(f'[xmltv] Downloading {label} EPG...')
                resp = requests.get(url, timeout=30, headers={'User-Agent': UA})
                xml_gz = resp.content
                with open(cache_file, 'wb') as f:
                    f.write(xml_gz)
                log(f'[xmltv] {label}: {len(xml_gz)//1024}KB')
            except Exception as e:
                log(f'[xmltv] {label} download error: {e}')
                continue
        try:
            xml_parts.append(gzip.decompress(xml_gz).decode('utf-8', errors='replace'))
        except Exception as e:
            log(f'[xmltv] {label} decompress error: {e}')
    ...
    # Match each DLTV channel name to an XMLTV ID
    cid_to_xmltv = {}
...
def _fetch_xmltv_background(cid_name_pairs):
    """Return existing EPG cache without blocking; start a background refresh if stale."""
    cached = {}
    try:
...
def list_by_language():
    ...
    xbmcgui.Dialog().notification('DLTV', 'Loading language index...', ICON, 2000)
    ch_list = channels()
    if not ch_list:
        closeDir()
        return
    ...
    if not index:
        xbmcgui.Dialog().notification('DLTV', 'iptv-org index unavailable', ICON, 4000)
        closeDir()
        return
    lang_counts = {}
    unclassified = 0
    ...
    if unclassified:
        addDir(f'[Unclassified]  ({unclassified})', build_url({'mode': 'lang_channels', 'lang': '__none__'}), True)
    closeDir()
    ...
    for href, name in ch_list:
        ...
        if lang_code == '__none__':
            ...
            if ch_lang != lang_code:
                continue
        fav_label = '★ Remove from favorites' if cid in fav_ids else '☆ Add to favorites'
...
def list_gen():
    chData = channels()
    if not chData:
        xbmcgui.Dialog().notification('DLTV', 'Failed to load channels. Check your connection.', ICON, 5000)
...
def _lookup_channel_name(cid):
    """Return real channel name for given ID string, or None if not found."""
    url = abs_url('24-7-channels.php')
    try:
        resp = fetch_via_proxy(url, headers={'Referer': get_active_base(), 'User-Agent': UA})
    except Exception:
        return None
    for m in _CARD_RX.finditer(resp):
        if m.group('id') == cid:
            name = html.unescape(m.group('title').strip()) or html.unescape(m.group('data_title').strip())
...
def channels():
    ...
    try:
        ...
        log(f"[DLTV] channels(): request failed: {e}")
        return []
...
def generate_m3u():
    """Parse channel list and write M3U to special://userdata/dltv.m3u.
    ...
    for m in card_rx.finditer(resp):
        ...
        play_url = ('plugin://plugin.video.dltv/?' +
    ...
    out_path = xbmcvfs.translatePath('special://userdata/dltv.m3u')
    try:
...
def pvr_setup():
    ...
    xbmcgui.Dialog().notification('DLTV', 'Generating M3U file…', ICON, 2000)
    out_path = generate_m3u()
    if not out_path:
        xbmcgui.Dialog().ok('DLTV — Export M3U',
    ...
    pvr_status = ('PVR IPTV Simple Client detected!' if pvr_installed
                  else 'PVR IPTV Simple Client not installed.\nInstall it from Extensions → Kodi Repository.')
    msg = (f'[B]{pvr_status}[/B]\n\n'
           f'M3U file generated:\n[I]{out_path}[/I]\n\n'
           f'Setup:\n'
           f'1. Settings → PVR IPTV Simple Client\n'
           f'2. M3U Play List URL → Local file\n'
           f'3. Path → {out_path}\n'
           f'4. OK → Kodi restarts PVR')
    if pvr_installed:
        if xbmcgui.Dialog().yesno('DLTV — Export M3U', msg,
                                   nolabel='Close', yeslabel='Open PVR settings'):
            xbmc.executebuiltin('Addon.OpenSettings(pvr.iptvsimple)')
    else:
        xbmcgui.Dialog().ok('DLTV — Export M3U', msg)
...
def _probe_hls_url(url):
    ...
    try:
        ...
        return None   # timeout = inconclusive, don't mark dead
    except requests.exceptions.ConnectionError:
        return None   # proxy unreachable — inconclusive, don't mark channel dead
    except Exception:
        return False
...
def _decode_host(url):
    """Rewrite dlstreams.top -> dlstreams.st for DNS resolution reliability."""
    if 'dlstreams.top' in url:
        return url.replace('dlstreams.top', 'dlstreams.st')
    return url

def _try_player_page(channel_id, player_url, watch_url, sess):
    """Try to extract a stream URL from a single player page.
    Returns (url, origin) tuple on success, None otherwise.
    origin is the required HTTP Origin header value, or None if not needed."""
    try:
        ...
        if r.status_code != 200:
            ...
            return None
        html = r.text
        # ── URL rewrite: dlstreams.top -> dlstreams.st (same IP, reliable DNS) ──────
        html = _decode_host(html)
        # ── CHEVY/auth-player guard: detect by content, not domain ──────────
        # Don't bail out — some pages embed CHEVY alongside another player.
        # The iframe scan below already skips /premiumtv/ iframes explicitly.
        if 'ksohls.ru' in html or '/premiumtv/daddyhd.php' in html:
            log(f'[AnyPlayer] CHEVY embed detected — scanning for alternative streams: {player_url}')
        ...
        for mm in _AP_M3U8_RX.finditer(html):
            ...
            try:
                ...
                if rp.status_code == 200 and '#EXTM3U' in rp.text[:200]:
                    log(f'[AnyPlayer] direct m3u8 OK: {candidate[:80]}')
                    return (candidate, None)
                log(f'[AnyPlayer] direct m3u8 skip ({rp.status_code}): {candidate[:60]}')
            except Exception:
                log(f'[AnyPlayer] direct m3u8 unreachable: {candidate[:60]}')

        # ── Generic: scan iframes for .m3u8 or streamUrl ────────────────────
        for iframe_url in _AP_IFRAME_RX.findall(html):
            if any(skip in iframe_url for skip in _AP_SKIP_HOSTS) or '/premiumtv/' in iframe_url:
                continue
            ...
            try:
                ...
                for mi in _AP_M3U8_RX.finditer(ri.text):
                    ...
                    try:
                        ...
                        log(f'[AnyPlayer] iframe m3u8 unreachable: {candidate[:60]}')
                ...
                # ── lovecdn-style: ?stream= param + /player/ token (domain-agnostic) ──
                m_stream = re.search(r'[?&]stream=([\w-]+)', iframe_url)
                if m_stream:
                    stream_name = m_stream.group(1)
                    m_lt = re.search(
                        r'(https?://[^/"\'\s]+/player/' + re.escape(stream_name) + r')["\'\s\\]',
                        ri.text)
                    lt_url = m_lt.group(1) if m_lt else f'https://lovetier.bz/player/{stream_name}'
                    r_lt = sess.get(lt_url, headers={'User-Agent': UA, 'Referer': iframe_url}, timeout=8)
                    if r_lt.status_code == 200:
                        m_su = re.search(r'streamUrl\s*:\s*"([^"]+)"', r_lt.text)
                        if m_su:
                            su = m_su.group(1).replace('\\/', '/')
                            try:
                                rp = sess.get(su, headers={'User-Agent': UA, 'Referer': lt_url}, timeout=5)
                                if rp.status_code == 200 and '#EXTM3U' in rp.text[:200]:
                                    _ensure_m3u8_proxy()
                                    port = _actual_proxy_port or M3U8_PROXY_PORT
                                ...
                                log(f'[AnyPlayer] lovecdn streamUrl unreachable for {stream_name} — skip')
                    else:
                        log(f'[AnyPlayer] lovecdn token HTTP {r_lt.status_code} for {stream_name}')

                # ── wiki.js player (fid + wiki.js src) — fully domain-agnostic ──
                mf = re.search(r'fid\s*=\s*"([^"]+)"', ri.text)
                mw = re.search(r'src=["\'](?:https?:)?//([^/"\']+)/wiki\.js["\']', ri.text)
                if mf and mw:
                    fid = mf.group(1)
                    wiki_host = mw.group(1)
                    st_url = f'https://{wiki_host}/wiki.php?player=desktop&live={fid}'
                    rst = sess.get(st_url, headers={'User-Agent': UA, 'Referer': iframe_url}, timeout=8)
                    if rst.status_code == 200:
                        mca = re.search(r'return\s*\(\[([^\]]+)\]\.join\(', rst.text)
                        if mca:
                            chars = re.findall(r'"(\\?.)"', mca.group(1))
                            url = ''.join(c.replace('\\/', '/') for c in chars)
                ...
                # lovecdn-style fallback: if iframe fetch failed but URL has ?stream=,
                # try lovetier.bz directly (lovecdn.ru may be down).
                m_stream_fb = re.search(r'[?&]stream=([\w-]+)', iframe_url)
                if m_stream_fb:
                    stream_name = m_stream_fb.group(1)
                    lt_url = f'https://lovetier.bz/player/{stream_name}'
                    try:
                        r_lt = sess.get(lt_url, headers={'User-Agent': UA, 'Referer': iframe_url}, timeout=8)
                        if r_lt.status_code == 200 and re.search(r'streamUrl\s*:\s*"https?:', r_lt.text):
                            _ensure_m3u8_proxy()
                            port = _actual_proxy_port or M3U8_PROXY_PORT
                            log(f'[AnyPlayer] lovecdn fallback (lovetier direct): stream={stream_name}')
                            return (f'http://127.0.0.1:{port}/lovecdn-ts/{quote_plus(stream_name)}', None)
                        log(f'[AnyPlayer] lovecdn fallback lovetier HTTP {r_lt.status_code} stream={stream_name}')
                    except Exception as e2:
                        log(f'[AnyPlayer] lovecdn fallback lovetier error stream={stream_name}: {e2}')
...
def get_stream_page_url(channel_id):
    ...
    try:
        ...
        sess = _get_session()
        r = sess.get(stream_url_page, headers={
            'User-Agent': UA,
            'Referer': watch_url,
        }, timeout=12)
        if r.status_code != 200:
        ...
        r2 = sess.get(player_url, headers={
            'User-Agent': UA,
            'Referer': stream_url_page,
        }, timeout=8)
        player_base = re.sub(r'^(https?://[^/]+).*', r'\1', player_url)
        auth_token, channel_salt = _extract_creds_with_js(r2.text, player_url, player_base, sess)
        if auth_token and channel_salt:
...
def get_direct_hls_url(channel_id):
    ...
    try:
        ...
        sess = _get_session()
        r = sess.get(stream_page, headers={'User-Agent': UA, 'Referer': watch_url}, timeout=12)
        if r.status_code != 200:
            log(f'[DirectHls] stream page HTTP {r.status_code} for id={channel_id}')
            return None, None
        mp = re.search(r'<iframe[^>]+src="(https?://[^"]+premiumtv[^"]+)"', r.text)
        if not mp:
            log(f'[DirectHls] no premiumtv iframe for id={channel_id}')
            return None, None
        player_url = mp.group(1)
        referer_base = re.sub(r'^(https?://[^/]+).*', r'\1', player_url)
        r2 = sess.get(player_url, headers={'User-Agent': UA, 'Referer': stream_page}, timeout=8)
        if r2.status_code != 200:
        ...
        try:
            ...
            if r3.status_code == 200 and '#EXT-X-STREAM-INF' in r3.text:
                for _sl in r3.text.splitlines():
                    _sl = _sl.strip()
                    if _sl and not _sl.startswith('#'):
                        hls_url = _sl if _sl.startswith('http') else urljoin(hls_url, _sl)
                        log(f'[DirectHls] resolved variant: {hls_url[:100]}')
                        break
        except Exception as e:
...
def _wide_fresh_token(slug, fallback_token=''):
    """Return a valid wideiptv token for slug, refreshing via the API when stale."""
    ...
    try:
        resp = requests.post(
            ...
            json={'channel': slug, 'current_token': current},
            headers={'User-Agent': UA, 'Referer': f'https://wideiptv.top/player/{slug}'},
            timeout=8,
        )
        data = resp.json()
        if data.get('success') and data.get('token'):
            with _wide_token_lock:
                _wide_token_cache[slug] = (data['token'], time.time() + int(data.get('expires_in', 300)))
            return data['token']
    except Exception as e:
...
def get_wideiptv_url(channel_id):
    ...
    try:
        ...
        sess = _get_session()
        r = sess.get(player_page, headers={'User-Agent': UA, 'Referer': watch_url}, timeout=12)
        if r.status_code != 200:
            log(f'[WideIptv] player page HTTP {r.status_code} for id={channel_id}')
            return None
        m = re.search(r'<iframe[^>]+src="(https?://[^"]*wideiptv[^"]*)"', r.text)
        if not m:
            log(f'[WideIptv] no wideiptv iframe for id={channel_id}')
            return None
        daddy_url = m.group(1)
        r2 = sess.get(daddy_url, headers={'User-Agent': UA, 'Referer': player_page}, timeout=8)
        if r2.status_code != 200:
            log(f'[WideIptv] daddy page HTTP {r2.status_code}')
            return None
        m2 = re.search(r'<iframe[^>]+src="(https?://[^"]*wideiptv[^"]*)"', r2.text)
        if not m2:
            log(f'[WideIptv] no player iframe on daddy page')
            return None
        player_url = m2.group(1)
        r3 = sess.get(player_url, headers={'User-Agent': UA, 'Referer': daddy_url}, timeout=8)
        if r3.status_code != 200:
            log(f'[WideIptv] player page HTTP {r3.status_code}')
            return None
        mc = re.search(r'streamUrl:\s*"([^"]+)"', r3.text)
        if not mc:
            log('[WideIptv] no streamUrl in player config')
            return None
        stream_url = mc.group(1).replace('\\/', '/')
        mt = re.search(r'currentToken:\s*"([^"]+)"', r3.text)
...
def PlayStream(link):
    try:
        ...
        if not channel_id:
            ...
            xbmcgui.Dialog().notification('DLTV', 'Channel not found (missing ID)', ICON, 4000)
        ...
        if _premiumtv_hls:
            # HLS via inputstream.adaptive: Kodi re-polls the live playlist and
            # downloads segments through the /raw/ proxy (premiumtv Referer applied).
            ...
            # HLS with AES-128: open_mode=ffmpeg so FFmpeg polls live playlist AND
            # handles AES-128-CBC decryption. open_mode=curl only fetches M3U8 once
            # (no live re-poll) which causes stream to stall after initial segments.
            liz.setMimeType('application/vnd.apple.mpegurl')
            liz.setProperty('inputstream', 'inputstream.ffmpegdirect')
            liz.setProperty('inputstream.ffmpegdirect.is_realtime_stream', 'true')
            liz.setProperty('inputstream.ffmpegdirect.open_mode', 'ffmpeg')
        elif '/lovecdn-ts/' in m3u8_url:
            # lovecdn via TS proxy: internal HLS client downloads segments and forwards
            # as a continuous MPEG-TS byte stream — no HLS re-polling, no loop risk.
            liz.setMimeType('video/MP2T')
        ...
        xbmcgui.Dialog().notification('DLTV', f'Error: {str(e)[:80]}', ICON, 6000)
        xbmcplugin.setResolvedUrl(addon_handle, False, xbmcgui.ListItem())
...
def Search_Channels():
    ...
    for href, title in chData:
        if term in title.lower():
            ...
            cid = cid_m.group(1) if cid_m else ''
            fav_label = '★ Remove from favorites' if cid in fav_ids else '☆ Add to favorites'
            ctx = [(fav_label, 'RunPlugin(%s)' % build_url({'mode': 'toggle_fav', 'cid': cid, 'name': title}))] if cid else []
            label = f'{_ch_label(title, cid, dead_cids)} ({cid})' if cid else title
            addDir(label, build_url({'mode': 'play', 'url': abs_url(href)}), False, context_menu=ctx)
    closeDir()
...
def Search_Channel_By_Number():
    ...
    if not channel_id.isdigit():
        xbmcgui.Dialog().notification('DLTV', 'Invalid number', ICON, 3000)
        xbmcplugin.endOfDirectory(addon_handle, False)
        return
...
def load_extra_channels(force_reload=False):
    ...
    try:
        ...
        xbmcgui.Dialog().notification('DLTV', 'Extra channels unavailable', ICON, 3000)
        return {}
...
def ExtraChannels_Play(url, name='Extra Channel', logo=ICON):
    ...
    try:
        ...
        xbmcgui.Dialog().notification("DLTV", "Failed to play channel", ICON, 3000)
...
if not params.get('mode'):
    ...
    if mode == 'menu':
        ...
        cid = xbmcgui.Dialog().input('Channel ID to add', type=xbmcgui.INPUT_NUMERIC)
        if cid and cid.strip().isdigit():
            cid = cid.strip()
            favs = get_favorites()
            if any(f['id'] == cid for f in favs):
                xbmcgui.Dialog().notification('DLTV', f'Channel {cid} already in favorites', ICON, 2000)
            else:
                real_name = _lookup_channel_name(cid) or f'Channel {cid}'
                favs.append({'id': cid, 'name': real_name})
                save_favorites(favs)
                xbmcgui.Dialog().notification('DLTV', f'Added: {real_name}', ICON, 3000)
                xbmc.executebuiltin('Container.Refresh')
        xbmcplugin.endOfDirectory(addon_handle, False)
        ...
        favs = get_favorites()
        if favs:
            xbmcgui.Dialog().notification('DLTV', f'Checking {len(favs)} channels…', ICON, 2000)
